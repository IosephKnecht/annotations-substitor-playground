...
import io.github.iosephknecht.parameterized.list.AwareParametrization
...
internal abstract class ProcessAndroidTestAnnotations2 : DefaultTask() {
        ...
        allDirectories.get().forEach { directory ->
            directory.asFile.walk().forEach { file ->
                if (file.isFile && file.name.endsWith(".class")) {
                    ...
                    val parameterizedMethods = ctClass
                        .methods
                        .asSequence()
                        .filter { it.availableAnnotations.any { it is Parameterized.Parameters } }
                        .toList()

                    if (parameterizedMethods.isNotEmpty()) {
                        parameterizedMethods.forEach { ctMethod ->
                            val methodInfoGetEid = ctMethod.getMethodInfo()
                            val constPool = methodInfoGetEid.constPool

                            val awareParameterizedAnnotation = Annotation(
                                AwareParametrization::class.java.name,
                                constPool
                            )

                            awareParameterizedAnnotation.addMemberValue(
                                "value",
                                IntegerMemberValue(constPool, 100)
                            )
                            val attributeNew = ctMethod
                                .methodInfo
                                .getAttribute(AnnotationsAttribute.visibleTag)
                                as AnnotationsAttribute
//                            val attributeNew = AnnotationsAttribute(
//                                constPool,
//                                AnnotationsAttribute.visibleTag
//                            )

                            attributeNew.addAnnotation(awareParameterizedAnnotation)
                            ctMethod.methodInfo.addAttribute(attributeNew)
                        }
...
//
//                        val awareParameterizedAnnotation = Annotation(
//                            AwareParametrization::class.java.canonicalName,
//                            classFileConstPool
//                        )
//
//                        awareParameterizedAnnotation.addMemberValue("value", IntegerMemberValue(classFileConstPool, 100))
//
//                        annotationsAttribute.addAnnotation(awareParameterizedAnnotation)
//
